 ...
 *
 */
int current_hours_first_digit = 0;
int current_hours_second_digit = 0;
int current_minutes_first_digit = 0;
int current_minutes_second_digit = 0;
int current_hours = 0;
...
char ADCState = 0; //Busy state of the ADC
int16_t ADCResult = 0; //Storage for the ADC conversion result
void print_message(char * msg, int chars_to_send) {
    uint8_t interruptStatus;
    int chars_sent = 0;
    done = 0;
//    EUSCI_A_UART_transmitBreak(EUSCI_A0_BASE);
    __delay_cycles(3000);
    EUSCI_A_UART_transmitData(EUSCI_A0_BASE, (uint8_t)msg[chars_sent]);
    chars_sent++;
    while (chars_sent != chars_to_send) {
        interruptStatus = EUSCI_A_UART_getInterruptStatus(EUSCI_A0_BASE, EUSCI_A_UART_TRANSMIT_COMPLETE_INTERRUPT_FLAG);
        if (interruptStatus){
...
void main(void)
{
    ...
    print_message("Please enter the current time HHMM\r\n", 38);
    while(status == 0){}
...
void EUSCIA0_ISR(void)
{
    ...
    if (RxStatus)
    {
        char received_bit[2]= (char)EUSCI_A_UART_receiveData(EUSCI_A0_BASE);
        strcat(current_time, &(received_bit[0]));
        if (strlen(current_time) == 2){
            strcat(current_time, ":");
        }
        if (strlen(current_time) == 5) {
            char string_to_print[1024];
            strcat(string_to_print, received_bit);
            strcat(string_to_print, "\n\r");
            strcat(string_to_print, "The current time is: ");
            strcat(string_to_print, current_time);
            strcat(string_to_print, "\n\r");
            current_hours_first_digit = ((int)current_time[0]) - 48;
            current_hours_second_digit = ((int)current_time[1]) - 48;
            current_minutes_first_digit = ((int)current_time[3]) - 48;
            current_minutes_second_digit = ((int)current_time[4]) - 48;
            current_hours = current_hours_first_digit * 10 + current_hours_second_digit;
            current_minutes = current_minutes_first_digit * 10 + current_minutes_second_digit;
            print_message(string_to_print, strlen(string_to_print));
            status = 1;


        }
        else {
            print_message(received_bit, 1);
            print_message("\n\r" , 2);
        }
...
void RTC_ISR (void)
{
    showChar((current_hours % 100 - current_hours % 10)/10 + '0', pos1);
    showChar((current_hours % 10) + '0', pos2);
    showChar((current_minutes % 100 - current_minutes % 10)/10 + '0', pos3);
    showChar((current_minutes % 10) + '0', pos4);
    showChar((current_seconds % 100 - current_seconds % 10)/10 + '0', pos5);
    showChar((current_seconds % 10) + '0', pos6);
    current_seconds++;
